...
import '../Fees/Fees.scss';
import MonthlyCustomPlan from '../Fees/MonthlyCustomPlan';
import OneTimeCharge from '../Fees/OneTimeCharge';
...
const AddFees = (props) => {
  ...
  const getSelectedBatches = (allbatches, selectBatches) => {
    ...
    console.log(selectBatches, 'hare raam');
  };

  const handleBatchesClose = () => {
    setSelectedBatches([]);
  ...
  const assignFeesToStudents = () => {
    ...
    post(payload, '/addMultipleUsersInWhitelist').then((responce) => {
...
export default connect(mapStateToProps)(AddFees);
