...
const Homework = (props) => {
  const { clientUserId, homework, searchString } = props;
  const [searchStringOutput, setSearchStringOuptut] = useState([]);

  useEffect(() => {
    const result = homework.filter((elem) => {
      return elem.test_name.toLowerCase().includes(searchString);
    });
    setSearchStringOuptut(result);
    // console.log(searchStringOutput);
  }, [searchString]);

  if (searchStringOutput.length > 0) {
    return (
      <section className='Tests__scrollableCard divContainer'>
        {searchStringOutput.map((elem) => {
          return (
            <div
              key={elem.test_id}
...
Homework.propTypes = {
  homework: PropTypes.arrayOf(PropTypes.number).isRequired,
